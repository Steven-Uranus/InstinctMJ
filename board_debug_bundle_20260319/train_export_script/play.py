...
def _build_generic_actor_onnx_policy(
    *,
    model_dir: Path,
    device: str,
):
    ...
    if not actor_path.exists():
        raise FileNotFoundError(f"Missing ONNX actor file: {actor_path}")

    ort_providers = ort.get_available_providers()
    actor = ort.InferenceSession(str(actor_path), providers=ort_providers)
    ...
    norm_eps: np.ndarray | None = None
    if normalizer_path.exists():
        normalizer_data = np.load(normalizer_path)
        norm_mean = normalizer_data["mean"].squeeze(0).astype(np.float32)
        norm_std = normalizer_data["std"].squeeze(0).astype(np.float32)
        norm_eps = normalizer_data["eps"].squeeze(0).astype(np.float32)
        print(f"[INFO] Loaded ONNX normalizer from: {normalizer_path}")
    else:
        print(f"[WARN] policy_normalizer.npz not found under {model_dir}; ONNX policy will use raw observations.")

    def _policy(obs: torch.Tensor) -> torch.Tensor:
        obs_np = obs.detach().cpu().numpy().astype(np.float32)
        if norm_mean is not None and norm_std is not None and norm_eps is not None:
            obs_np = (obs_np - norm_mean) / (norm_std + norm_eps)
        action_np = actor.run(None, {actor_input_name: obs_np})[0]
        return torch.from_numpy(action_np).to(device=device)
    ...
    if cfg.agent in {"zero", "random"}:
        ...
        if cfg.use_onnx:
            ...
            if "Parkour" in task_id:
                ...
                policy = _build_generic_actor_onnx_policy(
                    model_dir=onnx_model_dir,
                    device=device,
                )
